...
def generate_str_pdf(
    ...
    pdf.cell(0, 12, "SUSPICIOUS TRANSACTION REPORT", new_x="LMARGIN", new_y="NEXT", align="C")
    ...
    if cyber_events:
        ...
        for ev in cyber_events:
            ...
            pdf.cell(0, 5, f"  [{ts}] {event_type} — Device: {device}, Location: {location}, Anomaly: {score}", new_x="LMARGIN", new_y="NEXT")
    else:
    ...
    if txns:
        ...
        for tx in txns:
            ...
            pdf.cell(0, 5, f"  [{ts}] Rs.{amount:,.0f} {str(method).upper()} — {sender} -> {receiver}", new_x="LMARGIN", new_y="NEXT")
    else:
        pdf.set_font("Helvetica", "I", 10)
    ...
    if gemini_explanation:
        ...
        pdf.set_font("Helvetica", "", 10)
        pdf.set_text_color(50, 50, 70)
        pdf.multi_cell(0, 5, gemini_explanation)
        pdf.ln(4)
