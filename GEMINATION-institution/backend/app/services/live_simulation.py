...
from .timeseries_db import store_tick_event, get_tick, get_all_ticks, get_timeseries_stats
...
def generate_live_event() -> dict:
    ...
    if unified_score > 0.7:
        from .gemini_prompts import build_explainability_prompt
        gemini_prompt = build_explainability_prompt(
            tick=tick,
    ...
    # Store in timeseries database (ticks 1-90)
    store_tick_event(tick, result)
...
def generate_user_live_event(account_id: str) -> dict:
    ...
    if unified_score > 0.4 and changes:
        from .gemini_prompts import build_user_explainability_prompt_v2
        gemini_prompt = build_user_explainability_prompt_v2(
            tick=tick,
            account_id=account_id,
            unified_score=unified_score,
            cyber_score=cyber_score,
